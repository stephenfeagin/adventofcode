...
We now only need to determine how many IDs can be considered fresh. In order to do this, I first order the endpoints data by start. Then I compare the firsttwo rows of the endpoints data at a time and check whether their ranges overlap. If they do, I create a new row from the expanded range (i.e. old start to new end). If they do not, that range is added to a new data frame that contains only the non-overlapping ranges. I then get the length of each range and sum those lengths.
