...
We will use **dplyr**’s `lag()` function to offset each report and take
the difference. We then scrub the `NA` that results from the offset.
Finally, we check for monotonicity and boundedness, then return whether
both are true.
...
```
    ...
    Time difference of 0.01422906 secs

## Part 2

We now have to check if each report is safe if we remove any level. For
a report, we first check if it is safe to begin with. We then iterate
through the report removing one element at a time, checking if the
resulting vector is safe. If we get through the whole report without any
of them returning `TRUE`, we return `FALSE`.
...
```
    ...
    Time difference of 0.04367685 secs
